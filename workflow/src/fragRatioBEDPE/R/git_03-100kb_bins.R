...
write.table(QC_matrix,
            file.path(outdir, paste0(id, "_cov_stats.txt")),
...
# tib.list %>% head() %>% print.data.frame()
rm(AB)
write.table(tib.list,
            file.path(outdir, paste0(id, "_100kb_bins.txt")),
            sep = "\t", quote = F, row.names = F)

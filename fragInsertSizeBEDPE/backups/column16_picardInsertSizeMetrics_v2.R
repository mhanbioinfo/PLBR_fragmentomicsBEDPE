library(docopt)
doc = 'column16_picardInsertSizeMetrics_v2.R

Usage:
  column16_picardInsertSizeMetrics_v2.R --input INPUT --orientation ORIENTATION --output OUTPUT [ --histogram HISTOGRAM ] [ --histogram_width HISTOGRAM_WIDTH ]
...
# args$orientation = "FR"
# args$output = "/Users/minghan/bioinfoproj/fragmentomics/column16/TGL48_0011_Ct_T_PE_325_WG.merged.sorted.sub1pct.coordSortd_coordSortd.F2316_TLENgt0.NOTANDEMS.FRs.col16.InsertSizes.out"
...
# ## CORE is default
# CORE_RANGE = c(MEDIAN - DEVIATION*MAD, MEDIAN + DEVIATION*MAD)
# CORE_MEAN = mean(INSERT_VEC[INSERT_VEC > CORE_RANGE[1] & INSERT_VEC < CORE_RANGE[2]])
# CORE_STDV = sd(INSERT_VEC[INSERT_VEC > CORE_RANGE[1] & INSERT_VEC < CORE_RANGE[2]])
# ## if HISTOGRAM_WIDTH specified, then mean and stdv calculated according to HISTOGRAM_WIDTH
# HIST_WID_MEAN = mean(INSERT_VEC[INSERT_VEC <= HISTOGRAM_WIDTH])
# HIST_WID_STDV = sd(INSERT_VEC[INSERT_VEC <= HISTOGRAM_WIDTH])
...
while ((LOW >= MIN - 1 || HIGH <= MAX + 1) && PCT_COVERED < 0.9901){
  ## BIN is freq count
